...
export const loadAsmaUlHusna = async () => {
  try {
    const cached = await AsyncStorage.getItem('deen360_asma_cache');
    if (cached) {
    ...
    const liveData = await fetchAsmaLive();
    if (liveData) return liveData;
    
  } catch (e) {
    console.warn("Asma cache read error", e);
  }
...
const fetchAsmaLive = async () => {
  try {
    const res = await fetch('https://api.aladhan.com/v1/asmaAlHusna');
    const json = await res.json();
    if (json.code === 200 && json.data) {
      await AsyncStorage.setItem('deen360_asma_cache', JSON.stringify(json.data));
      return json.data;
    }
  } catch (e) {
